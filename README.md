...
```bash
# Look up the ground elevation of the site in a digital elevation model
python Utils/SiteElevation.py

# Print the position of the Sun and the circumstances of the eclipse at the site
python -m SolarTransit.Ephemeris

# Download the data and search for transits. The hourly files are cached in data/raw.
python -m SolarTransit.TransitSearch -w 12

# Plot the best candidates, or specific ones by callsign
python Utils/PlotTransits.py -n 3
python Utils/PlotTransits.py IBE0580

# The Sun is drawn flipped vertically by default, the way it appears in the eyepiece of a
# telescope. Other orientations are naked_eye, mirrored (a star diagonal) and inverted.
python Utils/PlotTransits.py IBE0580 --view naked_eye
...
```
$ python -m SolarTransit.TransitSearch -w 12 -n 5
...
```
SolarTransit/
    Config.py         site, time window, API access, thresholds
...
Utils/
    SiteElevation.py  elevation of the site from a digital elevation model
